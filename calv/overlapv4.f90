...
program pr_transferintegral
  ...
  double precision,allocatable :: S(:,:),e(:),c(:,:),work(:),SS(:,:)
...
!  write(*,*) flog1, flog2, dimerlog 
  ...
  !%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%!
  !% read information from flog1---rigid molecule--- noscf %!
  !%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%!
  ...
  allocate(c(nb1,nb1),s(nb1,nb1),c1(nb1))
  allocate(e(nb1))
  ...
  enddo
  call read_lowtri(s,nb1,99)

  do while(.True.)
    read(99,'(a11)') flagc
    if(flagc.eq.' Alpha MOs:' .or. flagc.eq.'     Molecu')  exit
  enddo
  if(flagc.eq.' Alpha MOs:') then 
      call read_MO(e,c,nb1,99)
  else if(flagc.eq.'     Molecu') then 
      call read_MO1(e,c,nb1,99)
  end if 
  close(99)
  c1all = c(1:nb1,imol1-9:imol1+10)
  deallocate(c,e)
  ...
  close(99)
  c2all= c(1:nb2,imol1-9:imol1+10)
  deallocate(c,e)



!  write(6,*) c1all(1:10,1:10)
!  write(6,*) s(1:10,1:10)
!  write(6,*) c2all(1:10,1:10)
!####################
  ...
  do orbt1=1,20
    ...
    do orbt2=1,20
      ...
      do ii=1,nb1
        ...
        enddo
      enddo
      if ( abs(overlap) >= abs(smax)) then
        smax=overlap
        nmax=imol2-10+orbt2  ! 1 for imol2-9
      end if 
      write(6,'(a,xi5,xi5,xf12.5)') "Overlap for ",imol1-10+orbt1,imol2-10+orbt2,overlap
    end do 
    write(6,'(a,xi5,xi5,xf12.5)') "orital1 max overlap with orbital2 and its overlap", imol1-10+orbt1,nmax,smax
  end do

  deallocate(c1all,c2all,c1,c2,S)
  stop
...
!  write(*,*) npblk,floor(real((n-1)/nblk))+1
  do ii=1,npblk
    read(iunt,*) cha
    read(iunt,*) cha  ! added for pop=full
    nline=nline+1
    jb=(ii-1)*nblk+1
    je=ii*nblk
    if(je>n) je=n
