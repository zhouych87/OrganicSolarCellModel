...
function fft3d(k,r,sc,method) !
    implicit none
    integer,intent(in)::k
    real, intent(in) :: r,sc
    character(15),intent(in)::method 
    real::fft3d, member,ttt
    ...
    if(k==0) then 
        ...
        if (trim(method)=="snsr") then  ! sin
            ...
            write(*,*) "Avialable Options:"
            write(*,*) "snsr snr sn snsqrtr and correponding cosine and abs"
            write(*,*) "As the value you given ",method,"is not support,"
            stop
        end if 
    end if 
    fft3d=member
...
SUBROUTINE morse3d(sc,method,c,elemt,nam,morU,morM,morV,morP,morE,morC,chg,nsc,npcs,myid,ierr)
use lib 
use MPI
implicit none 
integer,intent(in)::nam,nsc,npcs,myid,ierr
real,intent(out)::morU(nsc),morM(nsc),morV(nsc),morP(nsc),morE(nsc),morC(nsc)
real::mU(nsc),mM(nsc),mV(nsc),mP(nsc),mE(nsc),mC(nsc)
real,intent(in)::c(3,nam),sc
character(2),intent(in)::elemt(nam) 
character(15),intent(in)::method 
real,intent(in)::chg(1000)
integer::i,j,k
real::tc(3),l,r
...
morU=0.d0;morM=0.d0;morV=0.d0;morP=0.d0;morE=0.d0;morC=0.d0
mU=0.d0;mM=0.d0;mV=0.d0;mP=0.d0;mE=0.d0;mC=0.d0
...
do i=myid+2,nam,npcs
    do j=1,i-1
        ...
        r=sqrt(dot(tc,tc)) !  A
        do k =0,nsc-1
            member=fft3d(k,r,sc,method) !fft3d(k,r,sc,method) 
            mU(k+1)=mU(k+1)+member
            mM(k+1)=mM(k+1)+member*cwm
            mV(k+1)=mV(k+1)+member*cwv
            mP(k+1)=mP(k+1)+member*cwp
            mE(k+1)=mE(k+1)+member*cwe
            mC(k+1)=mC(k+1)+member*cwc
        end do
    end do
end do 
call MPI_Allreduce(mU, morU, nsc,MPI_REAL,MPI_SUM, MPI_COMM_WORLD,ierr)
call MPI_Allreduce(mM, morM, nsc,MPI_REAL,MPI_SUM, MPI_COMM_WORLD,ierr)
call MPI_Allreduce(mV, morV, nsc,MPI_REAL,MPI_SUM, MPI_COMM_WORLD,ierr)
call MPI_Allreduce(mP, morP, nsc,MPI_REAL,MPI_SUM, MPI_COMM_WORLD,ierr)
call MPI_Allreduce(mE, morE, nsc,MPI_REAL,MPI_SUM, MPI_COMM_WORLD,ierr)
call MPI_Allreduce(mC, morC, nsc,MPI_REAL,MPI_SUM, MPI_COMM_WORLD,ierr)
END SUBROUTINE 

!call morse3ddimer(sc,mform,c(:,1:an,1),elemt(1:an,1),an,c(:,1:an,2),elemt(1:an,2),an,morU,morM,morV,morP,morE,morC,tchg,tchg)
SUBROUTINE morse3ddimer(sc,method,c1,elemt1,nam1,c2,elemt2,nam2,morU,morM,morV,&
    & morP,morE,morC,chg1,chg2,nsc,npcs,myid,ierr)
use MPI
use lib 
implicit none 
integer,intent(in)::nam1,nam2,nsc,npcs,myid,ierr
real,intent(in)::c1(3,nam1),c2(3,nam2),sc
character(15),intent(in)::method 
character(2),intent(in)::elemt1(nam1),elemt2(nam2)
real,intent(in)::chg1(1000),chg2(1000)
real,intent(out)::morU(nsc),morM(nsc),morV(nsc),morP(nsc),morE(nsc),morC(nsc)
real::mU(nsc),mM(nsc),mV(nsc),mP(nsc),mE(nsc),mC(nsc)
integer::i,j,k,nr
real::tc(3),l,r
...
do i=myid+1,nam1,npcs
    do j=1,nam2
        ...
        cwc=chg1(i)*chg2(j)
        tc(1:3)=c1(1:3,i)-c2(1:3,j)
        r=sqrt(dot(tc,tc)) 
        do k =0,nsc-1
            member=fft3d(k,r,sc,method)
            mU(k+1)=mU(k+1)+member
            mM(k+1)=mM(k+1)+member*cwm
            mV(k+1)=mV(k+1)+member*cwv
            mP(k+1)=mP(k+1)+member*cwp
...
SUBROUTINE msnearest(c1,elemt1,nam1,c2,elemt2,nam2,morU,morM,morV,morP,morE,chg1,chg2,morC,nsc)
use lib 
implicit none 
integer,intent(in)::nam1,nam2,nsc
...
!!!!  CIP !!! 
!!!!!!!!!!!!!
SUBROUTINE calcip(nam1,c1,elemt1,nam2,c2,elemt2,chg1,chg2,cip,anglhist,rhist,npcs,myid,ierr)
use lib 
implicit none 
integer,intent(in)::nam1,nam2,npcs,myid,ierr
real,intent(in)::c1(3,nam1),c2(3,nam2)
character(2),intent(in)::elemt1(nam1),elemt2(nam2)
real,intent(in)::chg1(nam1),chg2(nam1)
real,intent(out)::cip(nam1,nam2)
integer,intent(out)::anglhist(100),rhist(100)
integer::i,j,nr
real::tc(3),r,cwc,ttt
real::cen(3),v1(3),v2(3),dpi,dr

! coordinate is in nm?
dpi=0.031415926
!dr=0.01 ! unit is nm discret is 0.1 Angstron
dr=0.1 ! unit is A
cip=0.d0
anglhist=0;cen=0.0
rhist=0;
...
do i =1,nam1
    do j=1,nam2
        ...
        v1=c1(1:3,i)-cen(:)
        v2=c2(1:3,j)-cen(:)
        r=acos(dot(v1,v2)/sqrt(dot(v1,v1)*dot(v2,v2)))  ! 0 ≤ ACOS(X) ≤ PI
        nr=floor(r/dpi)+1
        if (nr>100) nr=100 
        !print *, "NA Hist: ",nr
        anglhist(nr)=anglhist(nr)+1
        
    end do
end do 
END SUBROUTINE calcip


! 32th of nearest distance 
SUBROUTINE dnearest(c1,elemt1,nam1,c2,elemt2,nam2,mindis)
use lib 
implicit none 
...
character(2),intent(in)::elemt1(nam1),elemt2(nam2) 
integer::i,j,k
real::tc(3),l,r,rtmp,itmp,mindis(32)
character(10)::flname,mname,tmname
character(5)::telemnt
...
call calparam1r(indx1,crd1,natom1,outparam1)! LENGTH OF 1LONG 2SHRT, 3-4 width, 5AREA.6 angle of left and right plane 
call calparam1r(indx2,crd2,natom2,outparam2) ! 7-8 hieght of left and right plane

v1(:,1)=crd1(:,indx1(2))-crd1(:,indx1(1)) !LONG
v1(:,2)=crd1(:,indx1(4))-crd1(:,indx1(3)) !SHORT
v1(:,3)=crd1(:,indx1(3))-crd1(:,indx1(1)) !RIGHT
v1(:,4)=crd1(:,indx1(4))-crd1(:,indx1(1)) !LEFT 

v2(:,1)=crd2(:,indx2(2))-crd2(:,indx2(1))
v2(:,2)=crd2(:,indx2(4))-crd2(:,indx2(3))
v2(:,3)=crd2(:,indx2(3))-crd2(:,indx2(1))
v2(:,4)=crd2(:,indx2(4))-crd2(:,indx2(1))
...
vn1(:,1)=vn1(:,1)/sqrt(dot(vn1(:,1),vn1(:,1))) !normalize 
vn1(:,2)=cross(v1(:,4),v1(:,2)) !# normal of left plane  421
vn1(:,2)=vn1(:,2)/sqrt(dot(vn1(:,2),vn1(:,2))) !normalize 

vn2(:,1)=cross(v2(:,3),v2(:,1)) !# normal of right plane 321
vn2(:,1)=vn2(:,1)/sqrt(dot(vn2(:,1),vn2(:,1))) !normalize 
vn2(:,2)=cross(v2(:,4),v2(:,2)) !# normal of left plane  421
vn2(:,2)=vn2(:,2)/sqrt(dot(vn2(:,2),vn2(:,2))) !normalize 
...
outparam(3)=dot(param(:,3),param(:,1))/outparam(1)  !# width of right  A*B=|A||B|COS, H=ACOS=A*B/|B|
outparam(4)=dot(param(:,4),param(:,1))/outparam(1)  !# width of left
outparam(7)=dot(param(:,3),param(:,5))/outparam(5)  !# hight of right  A*B=|A||B|COS, H=ACOS=A*B/|B|
outparam(8)=dot(param(:,4),param(:,5))/outparam(5)  !# hight of left
END SUBROUTINE 
