...
!!ccccccc molecule-atom-distance-basis-core cccccccccccccccccc
      ...
      real::tdis,cell(3),kct,tdis1,tdis2(3),cutoffa
...
do m=1,nm 
    ...
    do k =-1,1  
       ...
       if (n/=m .and. tdis<cutoff**2 ) then !! to scan atom distance
             write(*,*) "cell i j k",m,n,i,j,k,sqrt(tdis)
             tdis1=1000  !!! inital minmun distance
             do iin=1,nam(n)
             do iim=1,nam(m)
                  ...
                  if (tdis<tdis1) tdis1=tdis  ! set minimum to tdis1 
             end do !iin
             end do !iim
!##################################################
!    selection based on nearest atom distance
!##################################################
            if (tdis1 < cutoffa**2) then 
                count(m)=count(m)+1
