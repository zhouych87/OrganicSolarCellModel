...
SUBROUTINE morse3ddimer(sc,method,c1,elemt1,nam1,c2,elemt2,nam2,morU,morM,morV,&
	& morP,morE,chg1,chg2,morC,cip,anglhist,rhist,rchist)
use lib 
implicit none 
...
do i =1,nam1
	do j=1,nam2
		...
		r=sqrt(dot(tc,tc)) ! 1 A - 10A, out of range will considered to be 1A or 10A
		nr=floor(r/dr)-10
		if (nr<1) then 
			nr=1
		else if (r>100) then 
		...
		v1=c1(1:3,i)-cen(:)
		v2=c2(1:3,j)-cen(:)
		l=acos(dot(v1,v2)/sqrt(dot(v1,v1)*dot(v2,v2)))  ! 0 ≤ ACOS(X) ≤ PI
		nr=floor(l/dpi)+1
		if (nr>100) nr=100 
		anglhist(nr)=anglhist(nr)+1

		if (present(chg1)) cip(i,j)=cwc/r  !! CIP
