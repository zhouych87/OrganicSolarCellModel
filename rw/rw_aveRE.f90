...
!cccc mpif90 -limf -lm rw.f CCCCCC
      Real(8),allocatable ::std(:),td(:,:),tt(:),p(:,:),conk(:,:),cen(:,:)
      real(8)::ttt,kct,tre(4,2),re(5)
...
if (myid .eq. 0) write(*,*) "   l(m^2)               t(s) "
if (myid .eq. 0) write(11,*) "   l(m^2)               t(s) "
